...
impl<E: Curve, H: Digest + Clone> RingPedersenStatement<E, H> {
	pub fn generate() -> (Self, RingPedersenWitness<E, H>) {
		...
		(
			Self { S: s, T: t, N: ek_tilde.clone().n, phi, ek: ek_tilde, phantom: PhantomData },
			RingPedersenWitness { p: dk_tilde.p, q: dk_tilde.q, lambda, phantom: PhantomData },
		)
	}
}
...
mod tests {
	...
	fn test_ring_pedersen() {
		...
		assert!(RingPedersenProof::<Secp256k1, Sha256, { crate::M_SECURITY }>::verify(
			&proof, &statement
		)
		.is_ok());
	}
