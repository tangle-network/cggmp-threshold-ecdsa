...
impl<E: Curve, H: Digest + Clone, const M: usize> RefreshMessage<E, H, M> {
	pub fn distribute(
		...
		let pdl_proof_vec: Vec<_> = (0..secret_shares.len())
			.map(|i| {
				let witness =
					PDLwSlackWitness { x: secret_shares[i].clone(), r: randomness_vec[i].clone() };
	...
	pub(crate) fn get_ciphertext_sum<'a>(
		...
		let ciphertext_sum = ciphertext_vec_at_indices_mapped
			.iter()
			.fold(Paillier::encrypt(ek, RawPlaintext::from(BigInt::zero())), |acc, x| {
				Paillier::add(ek, acc, x.clone())
			});
	...
	pub fn replace(
		...
		for old_party_index in old_to_new_map.keys() {
			let paillier_key =
				key.paillier_key_vec.get((old_party_index - 1) as usize).unwrap().clone();
			let h1_h2_n_tilde =
		...
		for new_party_index in paillier_key_h1_h2_n_tilde_hash_map.keys() {
			if *new_party_index <= current_len {
				...
				key.h1_h2_n_tilde_vec[(new_party_index - 1) as usize] =
					paillier_key_h1_h2_n_tilde_hash_map.get(new_party_index).unwrap().clone().1;
			} else {
		...
		for join_message in new_parties.iter() {
			...
			if party_index <= current_len {
				...
				key.h1_h2_n_tilde_vec
					.insert((party_index - 1) as usize, join_message.dlog_statement.clone());
			}
		}
	...
	pub fn collect(
		...
		for join_message in join_messages {
			...
			if join_message.dk_correctness_proof.verify(&join_message.ek, SALT_STRING).is_err() {
		...
		let new_share = Paillier::decrypt(&local_key.paillier_dk, cipher_text_sum).0.into_owned();
