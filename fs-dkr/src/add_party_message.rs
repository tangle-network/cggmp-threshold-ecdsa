...
/// Generates the parameters needed for the h1_h2_N_tilde_vec. These parameters
/// can be seen as environment variables for each party that they agree on. In
/// this case, each new party generates it's own DlogStatements and submits it's
/// proofs
...
fn generate_dlog_statement_proofs(
    ...
    let composite_dlog_proof_base_h2 =
        CompositeDLogProof::prove(&dlog_statement_base_h2, &xhi_inv);
...
impl<E: Curve, H: Digest + Clone, const M: usize> JoinMessage<E, H, M> {
    ...
    pub fn collect(
        ...
        let available_h1_h2_ntilde_vec: HashMap<u16, &DLogStatement> =
            refresh_messages
                .iter()
