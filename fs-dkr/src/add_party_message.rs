...
//! the new party. All parties pick an index and add the new ek to their LocalKey at the given
//! index.
...
use curv::{
	arithmetic::{BasicOps, Modulo, One, Samplable, Zero},
...
/// Generates the DlogStatement and CompositeProofs using the parameters generated by
...
fn generate_dlog_statement_proofs() -> (DLogStatement, CompositeDLogProof, CompositeDLogProof) {
	...
	(dlog_statement_base_h1, composite_dlog_proof_base_h1, composite_dlog_proof_base_h2)
}
...
impl<E: Curve, H: Digest + Clone, const M: usize> JoinMessage<E, H, M> {
	...
	/// The distribute phase for a new party. This distribute phase has to happen before the
	/// existing parties distribute. Calling this function will generate a JoinMessage and a pair of
	/// Paillier [Keys] that are going to be used when generating the [LocalKey].
	...
	/// tailored for a sent JoinMessage on which we assigned party_index. In this collect, a
	/// [LocalKey] is filled with the information provided by the [RefreshMessage]s from the other
	/// parties and the other join messages (multiple parties can be added/replaced at once).
	pub fn collect(
		&self,
		...
		let parameters = ShamirSecretSharing { threshold: new_t, share_count: new_n };
		...
		let new_share = Paillier::decrypt(&paillier_key.dk, cipher_text_sum).0.into_owned();
		...
		// check what parties are assigned in the current rotation and associate their
		// DLogStatements and check their CompositeDlogProofs.
		let available_h1_h2_ntilde_vec: HashMap<u16, &DLogStatement> = refresh_messages
			...
			.chain(join_messages.iter().map(|join_message| {
				(join_message.party_index.unwrap(), &join_message.dlog_statement)
			}))
			.collect();
