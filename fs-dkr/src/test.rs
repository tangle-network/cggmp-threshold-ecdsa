...
mod tests {
    ...
    fn test1() {
        ...
        assert_eq!(
            vss.reconstruct(
                &indices[..],
                &old_linear_secret_key[0..(t + 1) as usize]
            ),
            vss.reconstruct(
                &indices[..],
                &new_linear_secret_key[0..(t + 1) as usize]
            )
        );
    ...
    fn test_add_party_with_permute() {
        fn simulate_replace<const M: usize>(
            ...
            for (join_message, party_index) in
                join_messages.iter_mut().zip(party_indices)
            {
                join_message.party_index = Some(*party_index);
            }

            // each existing party has to generate it's refresh message aware of
            // the new parties
    ...
    fn simulate_dkr_removal<const M: usize>(
        ...
        for refresh_message in refresh_messages.iter() {
            broadcast_messages
                .insert(refresh_message.party_index.into(), Vec::new());
        }

        for refresh_message in refresh_messages.iter_mut() {
            if !remove_party_indices
                .contains(&refresh_message.party_index.into())
            {
                refresh_message.remove_party_indices =
                    remove_party_indices.clone();
            } else {
                let mut new_remove_party_indices = remove_party_indices.clone();
                new_remove_party_indices.retain(|value| {
                    *value != refresh_message.party_index.into()
                });
        ...
        for remove_party_index in remove_party_indices.iter() {
            assert_eq!(
                broadcast_messages[&(*remove_party_index as usize)].len(),
                1
            );
        }
    ...
    fn simulate_dkr<const M: usize>(
        ...
        for key in keys.iter_mut() {
            let (refresh_message, new_dk) =
                RefreshMessage::distribute(key.i, key, new_t, keys_len as u16)
                    .unwrap();
