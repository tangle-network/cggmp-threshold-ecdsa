...
mod tests {
	...
	use curv::{
		...
		elliptic::curves::{secp256_k1::Secp256k1Point, Secp256k1},
		BigInt,
	};
	use multi_party_ecdsa::protocols::multi_party_ecdsa::gg_2020::{
		...
		state_machine::{
			...
			sign::{CompletedOfflineStage, OfflineStage, SignManual},
		},
	};
	use sha2::Sha256;

	use crate::{add_party_message::JoinMessage, error::FsDkrResult};
	use curv::{
		cryptographic_primitives::{hashing::Digest, proofs::sigma_dlog::DLogProof},
	...
	fn test1() {
		...
		let old_linear_secret_key: Vec<_> =
			(0..old_keys.len()).map(|i| old_keys[i].keys_linear.x_i.clone()).collect();
	...
	fn test_add_party_with_permute() {
		fn simulate_replace<const M: usize>(
			...
			fn generate_join_messages_and_keys<const M: usize>(
				...
				(0..number_of_new_parties).map(|_| JoinMessage::distribute()).unzip()
			}
			...
			fn generate_refresh_parties_replace<const M: usize>(
				...
				keys.iter_mut()
					.map(|key| {
						RefreshMessage::replace(join_messages, key, old_to_new_map, key.t, new_n)
							.unwrap()
					})
					.unzip()
			}
			...
			let keys_replacements =
				new_keys_vec.iter().map(|a| a.1.clone()).collect::<Vec<LocalKey<Secp256k1>>>();
		...
		// check that sum of old keys is equal to sum of new keys
		let old_linear_secret_key: Vec<_> =
			(0..all_keys.len()).map(|i| all_keys[i].keys_linear.x_i.clone()).collect();
	...
	fn simulate_dkr_removal<const M: usize>(
		...
		for key in keys.iter_mut() {
			let (refresh_message, new_dk) =
				RefreshMessage::distribute(key.i, key, new_t, new_n).unwrap();
		...
		for refresh_message in refresh_messages.iter_mut() {
			...
			for (party_index, refresh_bucket) in broadcast_messages.iter_mut() {
				if refresh_message.remove_party_indices.contains(&(*party_index as u16)) {
					continue
				}
	...
	fn simulate_offline_stage(
		...
		for (i, &keygen_i) in (1..).zip(s_l) {
			simulation.add_party(
				OfflineStage::new(i, s_l.to_vec(), local_keys[usize::from(keygen_i - 1)].clone())
					.unwrap(),
			);
		}
