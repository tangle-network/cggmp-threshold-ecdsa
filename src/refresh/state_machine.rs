...
impl KeyRefresh {
    pub fn new(
        ...
        // `new_party_index_option`, otherwise returns an error if
        // neither is Some.
...
pub mod test {
    ...
    pub fn test_dkr_with_no_new_parties() {
        ...
        let mut new_local_keys =
            simulate_dkr_with_no_replacements(local_keys, &old_to_new_map);
    ...
    pub fn test_dkr_with_new_parties() {
        ...
        old_to_new_map.insert(5, 5);
        let old_local_keys = local_keys.clone();
        let new_local_keys = simulate_dkr_with_new_parties(
            local_keys,
            vec![6, 7],
            old_to_new_map,
            t,
    ...
    pub fn test_dkr_with_replace_parties() {
        ...
        let mut new_local_keys = simulate_dkr_with_replace_parties(
            local_keys,
            vec![2, 6],
            old_to_new_map,
            t,
    ...
    pub fn test_dkr_with_new_threshold() {
        ...
        assert_eq!(
            vss.reconstruct(
                &old_indices[..],
                &old_linear_secret_key[0..(init_t + 1) as usize]
            ),
            new_vss.reconstruct(
                &new_indices[..],
                &new_linear_secret_key[0..(new_t + 1) as usize]
            )
        );
        assert_ne!(old_linear_secret_key, new_linear_secret_key);
    }
