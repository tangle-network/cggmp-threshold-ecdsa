...
impl Keys {
	...
	pub fn phase1_broadcast_phase3_proof_of_correct_key_proof_of_correct_h1h2(
		...
		let dlog_statement_base_h1 =
			DLogStatement { N: self.N_tilde.clone(), g: self.h1.clone(), ni: self.h2.clone() };
		let dlog_statement_base_h2 =
			DLogStatement { N: self.N_tilde.clone(), g: self.h2.clone(), ni: self.h1.clone() };
		...
		let decom1 = KeyGenDecommitMessage1 { blind_factor, y_i: self.y_i.clone() };
		(bcm1, decom1)
	}
		...
		let correct_key_correct_decom_all = (0..bc1_vec.len())
			.map(|i| {
				...
				let test_res =
					...
					) == bc1_vec[i].com && bc1_vec[i]
						.correct_key_proof
						.verify(&bc1_vec[i].e, zk_paillier::zkproofs::SALT_STRING)
						.is_ok() && bc1_vec[i].e.n.bit_length() >= PAILLIER_MIN_BIT_LENGTH &&
		...
		let correct_ss_verify = (0..y_vec.len())
			.map(|i| {
				let res = vss_scheme_vec[i]
					...
					.is_ok() && vss_scheme_vec[i].commitments[0] == y_vec[i];
				if !res {
					bad_actors_vec.push(i);
					false
				} else {
...
impl PartyPrivate {
	...
	pub fn refresh_private_key(&self, factor: &Scalar<Secp256k1>, index: usize) -> Keys {
		...
		Keys { u_i: u, y_i: y, dk, ek, party_index: index, N_tilde, h1, h2, xhi, xhi_inv }
	}
...
impl SignKeys {
	...
	pub fn phase1_broadcast(&self) -> (SignBroadcastPhase1, SignDecommitPhase1) {
		...
		(
			...
			SignDecommitPhase1 { blind_factor, g_gamma_i: self.g_gamma_i.clone() },
		)
	}
	...
	pub fn phase2_sigma_i(
		...
		(0..vec_len).map(|i| &miu_vec[i] + &ni_vec[i]).fold(ki_w_i, |acc, x| acc + x)
	}
...
impl LocalSignature {
	pub fn phase5_proof_pdl(
		...
		let pdl_w_slack_witness = PDLwSlackWitness { x: k_i.clone(), r: k_enc_randomness.clone() };
	...
	pub fn output_signature(&self, s_vec: &[Scalar<Secp256k1>]) -> Result<SignatureRecid, Error> {
		...
		let r = Scalar::<Secp256k1>::from(
			&self.R.x_coord().unwrap().mod_floor(Scalar::<Secp256k1>::group_order()),
		);
...
pub fn verify(sig: &SignatureRecid, y: &Point<Secp256k1>, message: &BigInt) -> Result<(), Error> {
	...
	if sig.r ==
		Scalar::<Secp256k1>::from(
			&(gu1 + yu2).x_coord().unwrap().mod_floor(Scalar::<Secp256k1>::group_order()),
		) {
		Ok(())
	} else {
		Err(InvalidSig)
