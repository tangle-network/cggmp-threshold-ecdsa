use crate::utilities::mta::{
    range_proofs::tests::generate_init, MessageA, MessageB,
};
...
fn test_mta() {
    ...
    let (m_a, _) =
        MessageA::a(&alice_input, &ek_alice, &[dlog_statement.clone()]);
    let (m_b, beta, _, _) =
        MessageB::b(&bob_input, &ek_alice, m_a, &[dlog_statement]).unwrap();
    let alpha = m_b
