...
use paillier::{
	Add, Decrypt, DecryptionKey, Encrypt, EncryptionKey, Mul, Paillier, RawCiphertext, RawPlaintext,
};
use serde::{Deserialize, Serialize};
use sha2::Sha256;
use thiserror::Error;
use zk_paillier::zkproofs::{IncorrectProof, RangeProofNi};
...
impl Verifier {
	pub fn message1(statement: &PDLStatement) -> (PDLVerifierFirstMessage, PDLVerifierState) {
		...
		(
			PDLVerifierFirstMessage { c_tag: c_tag.clone(), c_tag_tag: c_tag_tag.clone() },
			PDLVerifierState { c_tag, c_tag_tag, a, b, blindness, q_tag, c_hat: BigInt::zero() },
		)
	}
...
impl Prover {
	...
	pub fn message2(
		...
		let ab_concat = &verifier_second_message.a +
			verifier_second_message.b.clone().shl(verifier_second_message.a.bit_length()); // b|a (in the paper it is a|b)
		let c_tag_tag_test =
