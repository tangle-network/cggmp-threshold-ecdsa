...
pub type Round0Messages =
    Store<BroadcastMsgs<SigningBroadcastMessage1<Secp256k1>>>;
pub type Round1Messages =
    Store<BroadcastMsgs<Option<SigningIdentifiableAbortMessage<Secp256k1>>>>;
...
mod test {
    ...
    fn simulate_sign(
        ...
        for (ssid, presigning_data) in inputs {
            simulation.add_party(
                Signing::new(ssid, l, m.clone(), presigning_data).unwrap(),
            );
        }
    ...
    pub fn generate_parties_and_simulate_sign(t: u16, n: u16, p: u16) {
        ...
        let (keys, ssids, presigning_outputs) =
            generate_parties_and_simulate_presign(t, n, p);
        assert_eq!(keys.len(), n as usize);
        assert_eq!(ssids.len(), p as usize);

        // Creates inputs for signing simulation based on test parameters and
        // pre-signing outputs.
        ...
        // Runs signing simulation for test parameters and verifies the output
        // signature.
