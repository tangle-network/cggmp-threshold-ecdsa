...
impl Round0 {
		...
		O: Push<Msg<Box<SigningBroadcastMessage1<Secp256k1>>>>,
	{
			...
			self.presigning_data.get_mut(&(self.l as u16))
		{
			...
			// sigma_i = k*m + r*chi
			let sigma_i = presigning_output.k_i.mul(&self.m).add(&r.mul(&presigning_output.chi_i)).mod_floor(&self.ssid.q);
...
impl Round1 {
		...
		O: Push<Msg<Box<Option<SigningIdentifiableAbortMessage<Secp256k1>>>>>,
	{
		...
		let sigma: BigInt =
			sigmas.values().into_iter().fold(self.sigma_i.clone(), |acc, x| acc.add(x)).mod_floor(&self.ssid.q);
